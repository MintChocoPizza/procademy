// Force_RST_Transmission_Server.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
int main()
{
	...
	/////////////////////////////////////////////////////////////////////////
	// 여기서 자신의 컴퓨터의 호스트 이름과 IP주소를 출력한다.
	// 
	/////////////////////////////////////////////////////////////////////////
	char hostname[256];
	struct addrinfo OreoPizza_Hints, * OreoPizza_result, * Oreopizza_Ptr;
	...
	// 클라이언트와 데이터 통신 
	int i_Send_Result;
	char Recv_BufF[DEFAULT_BUFF_LEN];
	int Recv_Buff_Len = DEFAULT_BUFF_LEN;
	...
	printf("Host name: %s\n", hostname);
	// IP 주소 가져오기
	ZeroMemory(&OreoPizza_Hints, sizeof(OreoPizza_Hints));
	OreoPizza_Hints.ai_family = AF_INET; // IPv4

	if (getaddrinfo(hostname, NULL, &OreoPizza_Hints, &OreoPizza_result) != 0)
	{
	...
	for (Oreopizza_Ptr = OreoPizza_result; Oreopizza_Ptr != NULL; Oreopizza_Ptr = Oreopizza_Ptr->ai_next)
	{
		sockaddr_ipv4 = (struct sockaddr_in*)Oreopizza_Ptr->ai_addr;
		inet_ntop(AF_INET, &sockaddr_ipv4->sin_addr, Ip_Str, sizeof(Ip_Str));
		printf("IPv4 주소: %s\n", Ip_Str);
		printf_s("port: %s \n", DEFAULT_PORT);
		printf_s("^^^^^^ 위 주소를 상대방에게 건내주세요 ^^^^^^ \n");
	}
	freeaddrinfo(OreoPizza_result);
