...
public:
	...
	//----------------------------------------------------------------------------------------
	// 아무리 찾아도 서로 물려놓고, 밖에서 할당하여 건내주는 방법 외에는 메모리 풀을 사용하는
	// 방법을 모르겠다......
	OreoPizza::CMemoryPool<Node<T>>* _MemPool;
...
public:
	CList(OreoPizza::CMemoryPool<Node<T>> &MemPool);
	~CList();
...
//----------------------------------------------------------------------------------------
template <typename T>
CList<T>::CList(OreoPizza::CMemoryPool<Node<T>>& MemPool) : _size(0), _MemPool(&MemPool)
{
	_head._Prev = NULL;
	_head._Next = &_tail;
...
void CList<T>::push_back(const T& data)
{
	...
	Node<T>* pNewNode = _MemPool->Alloc();
...
void CList<T>::pop_back(void)
{
	...
	Node<T>* removeNode = _tail._Prev;
	Node<T>* prevNode = removeNode->_Prev;
...
void CList<T>::pop_front(void)
{
	...
	Node<T>* removeNode = _head._Next;
	Node<T>* nextNode = removeNode->_Next;
...
{
	Node<T>* CurNode = _Where._node;
	Node<T>* NextNode = _Where._node->_Next;
	Node<T>* PreNode = _Where._node->_Prev;
