// MailSender2_2.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
int _tmain(int argc, TCHAR *argv[])
{
	...
	FILE* fileStream;
	errno_t err = _tfopen_s(&fileStream, _T("InheritableHandle.txt"), _T("rt"));
	if (err == 0)
	{
	...
	_ftscanf_s(fileStream, _T("%d"), &hMailSlot);
	fclose(fileStream);
	...
	while (1)
	{
		...
		if (!WriteFile(hMailSlot, message, _tcslen(message) * sizeof(TCHAR), &bytesWriten, NULL))
		{
