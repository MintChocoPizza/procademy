...
void TypeTextFile(void)
{
    ...
    if (!_tcscmp(cmdTokenList[2], _T("|")))
    {
        ...
        SECURITY_ATTRIBUTES pipSA = { sizeof(SECURITY_ATTRIBUTES), NULL, TRUE };

        CreatePipe(&hReadPipe, &hWritePipe, &pipSA, 0);
        ...
        _stprintf_s(cmdString, _T("%s %s"), cmdStringWithOptions, cmdTokenList[1]);
    }
}
...
int CmdProcessing(int tokenNum)
{
    ...
    else if (!_tcscmp(cmdTokenList[0], _T("sort")))
    {
        ...
        {
            SECURITY_ATTRIBUTES fileSec = { sizeof(SECURITY_ATTRIBUTES), NULL, TRUE };  // 상속 가능해야 지정 가능!
